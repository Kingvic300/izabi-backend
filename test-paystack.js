...
async function runTests() {
  try {
    ...
    try {
      const loginResponse = await apiRequest('/api/auth/login', 'POST', testUser);
    ...
    try {
      ...
      logSuccess(`Payment history retrieved: ${history.total} total payments`);
    ...
    try {
      const stats = await apiRequest('/api/payments/stats', 'GET', null, true);
    ...
    try {
      const banks = await apiRequest('/api/payments/banks/list', 'GET', null, true);
      logSuccess(`Retrieved ${banks.data?.length || 0} banks`);
      if (banks.data && banks.data.length > 0) {
        logInfo(`Sample bank: ${banks.data[0].name} (${banks.data[0].code})`);
      }
    } catch (error) {
    ...
    log('  4. Test refund functionality (requires successful payment)', 'blue');
