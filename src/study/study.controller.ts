...
export class StudyController {
  ...
  async generateQuestions(
    ...
    return this.studyService.generateMaterial(req.user.userId, file, 'quiz', {
      count,
    });
  }
  ...
  async ingestDirect(
    ...
    if (!file)
      throw new BadRequestException('File is required for direct ingestion.');
  ...
  async processPDFSection(
    ...
    this.processPDFSectionBackground(
      ...
      console.error(
        `[StudyController] Background PDF section processing failed:`,
        err,
      );
    });
  ...
  private async processPDFSectionBackground(
    ...
    try {
      ...
      console.error(
        `[StudyController] PDF section background processing failed:`,
        error,
      );
      const job = await this.studyService.getJobStatus(jobId);
      if (job) {
