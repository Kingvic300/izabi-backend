    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        return next.handle().pipe(
            ...
            catchError((error) => {
                ...
                const eventId = uuidv4();
                this.logAction(context, 'FAILURE', eventId, error).catch(
                    (err) =>
                        console.error(
                            '[AuditInterceptor] Failure log failed',
                            err,
                        ),
                );
        ...
        if (jwtUser?.userId) {
            try {
                fullUser = (await this.userModel
                    ...
                    .select(
                        'firstName lastName email role plan isVerified createdAt',
                    )
                    .lean()
                    .exec()) as UserDocument;
            } catch (e) {
                console.error(
                    '[AuditInterceptor] Failed to fetch user details',
                    e,
                );
            }
        }
