...
export class AuditScheduler {
  ...
  async handleMediumDigest(isExternal = false) {
    ...
    this.logger.log(`Running MEDIUM digest (Trigger: ${isExternal ? 'External' : 'Cron'})...`);
    ...
    try {
      ...
      await this.updateLastRun('medium-digest', 'SUCCESS', `Sent ${events.length} events`);
  ...
  async handleDailyLowDigest(isExternal = false) {
    ...
    this.logger.log(`Running Daily LOW digest (Trigger: ${isExternal ? 'External' : 'Cron'})...`);
    ...
    try {
      ...
      await this.updateLastRun('low-digest', 'SUCCESS', `Sent ${events.length} events`);
      return { status: 'success', sent: events.length };
    } catch (error) {
  ...
  private async shouldRunJob(jobName: string, intervalMs: number): Promise<boolean> {
  ...
  private async updateLastRun(jobName: string, status: 'SUCCESS' | 'FAILURE', details: string) {
    await this.cronLogModel.findOneAndUpdate(
      { jobName },
      { 
        lastRunAt: new Date(),
        status,
  ...
  private async sendDigest(subject: string, events: AuditLogDocument[], description: string) {
    ...
    let html = `
      ...
      <ul>
        ${Object.entries(grouped).map(([action, count]) => `<li>${action}: ${count}</li>`).join('')}
      </ul>
