...
export class QuizService {
    ...
    private async checkQuickTestCooldown(userId: string) {
        ...
        if (lastTest && (lastTest as any).createdAt) {
            const timeSince = Date.now() - new Date((lastTest as any).createdAt).getTime();
            if (timeSince < this.QUICK_TEST_COOLDOWN_MS) {
                const waitMinutes = Math.ceil((this.QUICK_TEST_COOLDOWN_MS - timeSince) / 60000);
                throw new BadRequestException(`Please wait ${waitMinutes} minutes before starting another Quick Test.`);
            }
        }
    }
    ...
    private async gatherUserContent(userId: string): Promise<string> {
        const [notes, summaries] = await Promise.all([
            this.noteModel.find({ userId }).sort({ updatedAt: -1 }).limit(5).exec(),
            this.studyModel.find({ userId, type: 'summary' }).sort({ createdAt: -1 }).limit(3).exec()
        ]);

        const contentParts: string[] = [];
        notes.forEach(n => contentParts.push(`[NOTE: ${n.title}]\n${n.content}`));
        summaries.forEach(s => s.summary && contentParts.push(`[SUMMARY: ${s.fileName}]\n${s.summary}`));
    ...
    async generateQuickTest(userId: string) {
        ...
        if (!userContent || userContent.trim().length < this.MIN_CONTENT_LENGTH) {
            throw new BadRequestException('Upload study materials or create notes first to generate a personalized Quick Test!');
        }
    ...
    async submitQuickTest(quizId: string, userId: string, answers: Record<string, string>) {
        ...
        if (quiz.status !== 'STARTED') throw new BadRequestException('Test already submitted or expired.');
        ...
        const results = quiz.questions.map((q: any) => {
            ...
            const isCorrect = this.compareAnswers(q.type, userAnswer, q.correctAnswer);
    ...
    private compareAnswers(type: string, user: string, correct: string): boolean {
        if (!user) return false;
        const norm = (s: string) => s.trim().toLowerCase();
        if (type === 'multiple_choice' || type === 'true_false') return norm(user) === norm(correct);
    ...
    async getDailyChallenge(userId: string) {
        const latestNote = await this.noteModel.findOne({ userId }).sort({ updatedAt: -1 }).exec();
        if (!latestNote) {
            return { success: false, message: 'Create a note to unlock daily challenges!' };
        }
    ...
    async getGenericPracticeQuestions(count: number = 5) {
        const bank = [
            { id: 'gen-1', question: "Which pattern comes next: 2, 4, 8, 16, ...?", options: ["20", "24", "32", "64"], answer: "32", explanation: "Multiplied by 2.", questionType: "multiple_choice" },
            { id: 'gen-2', question: "What is active recall?", options: ["Reading", "Testing yourself", "Highlighting", "Listening"], answer: "Testing yourself", explanation: "Retrieval practice is key.", questionType: "multiple_choice" }
        ];
